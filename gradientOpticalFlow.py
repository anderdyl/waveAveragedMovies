...
out2 = cv.VideoWriter('wam30sAverage_2sDT_GradientFilter.avi',cv.VideoWriter_fourcc(*'XVID'), 10, (frame_width,frame_height),0)

#out2 = cv.VideoWriter('wam30sAverage_2sDT_GradientFilter.avi',cv.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
...
#    flow = cv.calcOpticalFlowFarneback(prev=prev_gray, next=gray, flow=None, pyr_scale=0.5, levels=3, winsize=25, iterations=3, poly_n=7, poly_sigma=1.5, flags=0)
    ...
    #finalImg = draw_flow(gray.astype(np.uint8), flow, step=12)
    ...
    cv.imshow('flow',mag.astype(np.uint8))
    #cv.imshow("dense optical flow", rgb)
    # Updates previous frame
    out2.write(mag.astype(np.uint8))
